/**
 * @file  CustomSchedulers.hh
 *
 * @brief   DIET CustomScheduler class header
 *
 ...
 *   |LICENSE|
 */
#ifndef _CUSTOMSCHEDULER_HH_
#define _CUSTOMSCHEDULER_HH_
...
#define USERDEFINED(server, nb) (diet_est_get_internal(&(server).estim, \
                                                       EST_USERDEFINED + (nb), \
                                                       0))
#define CONSO(server) (diet_est_get_internal(&(server).estim, EST_CONSO, 0.0))

#define RANDOM(server) (rand() % 1000)

#define CUSTOM(server) (CONSO(server)/TIMESINCELASTSOLVE(server))
...
#define SORTFUN(compCustom, TIMESINCELASTSOLVE)
  bool						
  compCustom(const corba_server_estimation_t &r1,     
       const corba_server_estimation_t &r2) {
