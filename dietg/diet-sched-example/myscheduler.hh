 ...
 *
 * @brief   DIET ConsoScheduler class header
 *
...
#define USE USE_CONSO

int indice = 1;
/* conso | random | perf | custom */
...
#define USERDEFINED(server, nb) (diet_est_get_internal(&(server).estim, \
                                                       EST_USERDEFINED + (nb), \
                                                       0))
#define CONSO(server) (diet_est_get_internal(&(server).estim, EST_CONSOJOB, 0.0))
...
/*#define SORTFUN(compConso, CONSO)
  bool						
  compConso(const corba_server_estimation_t &r1,     
       const corba_server_estimation_t &r2) {   
    return CONSO(r2) < CONSO(r1);             
  }
*/
#if USE == USE_CONSO
#define REV_SORTFUN(compFunction, CONSO)
  bool						
  compFunction(const corba_server_estimation_t &r1,     
       const corba_server_estimation_t &r2) {   
    return CONSO(r1) < CONSO(r2);             
  }
#elif USE == USE_NODEFLOPS
  #define SORTFUN(compFunction, FLOPS)
  bool						
  compFunction(const corba_server_estimation_t &r1,     
       const corba_server_estimation_t &r2) {
    return NODEFLOPS(r1) < NODEFLOPS(r2);
  }
#elif USE == USE_PERF
  #define SORTFUN(compFunction, TIMESINCELASTSOLVE)
  bool						
  compFunction(const corba_server_estimation_t &r1,     
       const corba_server_estimation_t &r2) {
    srandom(time(NULL));   
    return TIMESINCELASTSOLVE(r2) < TIMESINCELASTSOLVE(r1);             
  }
#elif USE == USE_CUSTOM
  #define SORTFUN(compFunction, CUSTOM)
  bool						
  compFunction(const corba_server_estimation_t &r1,     
       const corba_server_estimation_t &r2) {   
    return CUSTOM(r2) < CUSTOM(r1);             
  }
#endif
